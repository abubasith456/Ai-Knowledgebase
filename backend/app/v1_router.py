...
for d in [PROJECTS_DIR, FILES_DIR, INDEX_DIR]:
	d.mkdir(parents=True, exist_ok=True)


# In-memory mapping from project to files and indexes (backed by simple JSON files)
...
def _list_project_files(project_id: str) -> List[Dict]:
	import json
	files: List[Dict] = []
	for meta_path in FILES_DIR.glob(f"{project_id}_*.json"):
		try:
			files.append(json.loads(meta_path.read_text()))
		except Exception:
			continue
	return files


def _save_file_meta(meta: Dict):
	import json
	file_id = meta["fileId"]
	project_id = meta["projectId"]
	(FILES_DIR / f"{project_id}_{file_id}.json").write_text(json.dumps(meta, indent=2))


def _read_file_meta(project_id: str, file_id: str) -> Dict:
	import json
	path = FILES_DIR / f"{project_id}_{file_id}.json"
	if not path.exists():
		raise HTTPException(status_code=404, detail="File not found")
	return json.loads(path.read_text())


def _delete_file_meta(project_id: str, file_id: str):
	for path in [FILES_DIR / f"{project_id}_{file_id}.json", FILES_DIR / f"{project_id}_{file_id}.bin"]:
		if path.exists():
			path.unlink()
...
def delete_project(projectId: str):
	...
	if not p.exists():
		raise HTTPException(status_code=404, detail="Project not found")
	# delete related files and indexes metadata (not vector data)
	for meta in FILES_DIR.glob(f"{projectId}_*.json"):
		meta.unlink()
...
# 2. FILE MANAGEMENT
@router.post("/projects/{projectId}/files", response_model=FileUploadResponse)
def upload_file(projectId: str, file: UploadFile = File(...)):
	_ensure_project_exists(projectId)
	file_id = f"file_{uuid.uuid4().hex[:8]}"
	filename = file.filename or "uploaded.bin"
	binary_path = FILES_DIR / f"{projectId}_{file_id}.bin"
	...
	return FileUploadResponse(fileId=file_id, projectId=projectId, status="uploaded")


@router.get("/projects/{projectId}/files", response_model=List[FileSummary])
def list_files(projectId: str):
	_ensure_project_exists(projectId)
	items = _list_project_files(projectId)
	return [FileSummary(fileId=i["fileId"], filename=i.get("filename", "")) for i in items]


@router.get("/projects/{projectId}/files/{fileId}", response_model=FileDetails)
def get_file_details(projectId: str, fileId: str):
	meta = _read_file_meta(projectId, fileId)
	return FileDetails(fileId=meta["fileId"], filename=meta.get("filename", ""), status=meta.get("status", "uploaded"))


@router.delete("/projects/{projectId}/files/{fileId}", response_model=DeleteFileResponse)
def delete_file(projectId: str, fileId: str):
	_read_file_meta(projectId, fileId)  # ensure exists
	_id = fileId
	_delete_file_meta(projectId, fileId)
	return DeleteFileResponse(status="deleted", fileId=_id)
...
def delete_index(projectId: str, indexId: str):
	...
	# Note: not deleting Chroma collection data for safety
...
def ingest_into_index(projectId: str, indexId: str, payload: IngestIntoIndexRequest):
	meta = _read_index_meta(projectId, indexId)
	file_meta = _read_file_meta(projectId, payload.fileId)
	binary_path = FILES_DIR / f"{projectId}_{payload.fileId}.bin"
	if not binary_path.exists():
		raise HTTPException(status_code=404, detail="File data missing")

	pages_text = parse_pdf_with_docling(str(binary_path))
	chunks = hybrid_chunk_document(
		pages_text=pages_text,
		max_tokens=8000,
		overlap_tokens=200,
		metadata={"projectId": projectId, "fileId": payload.fileId, "filename": file_meta.get("filename", meta["name"])},
	)
	...
	return IngestIntoIndexResponse(status="indexed", fileId=payload.fileId, indexId=indexId, chunks=len(ids))
...
def query_index(projectId: str, indexId: str, payload: QueryRequest):
	...
	for i, text in enumerate(docs):
		...
		answers.append(QueryAnswer(text=text, source=source_meta.get('fileId')))
...
def quick_test(file: UploadFile = File(...), query: str = Form("What is in this document?")):
	...
	embedder = get_embedding_function()
	embs = embedder([c.text for c in chunks])
	# Naive top-k from cosine distance via Chroma's Ephemeral in-memory
	client = _client()
