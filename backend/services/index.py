...
def create_document_index(chunks: List[str], metadata: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    Create an index structure for document chunks.
    ...
    Returns:
        Dictionary containing the index structure
    """
    index_data = {
        "chunks": chunks,
    ...
    for i, chunk in enumerate(chunks):
        chunk_meta = {
            ...
            "chunk_end_char": sum(len(chunks[j]) for j in range(i + 1))
        }
        index_data["chunk_metadata"].append(chunk_meta)
    
    return index_data

def merge_chunks_for_query(chunks: List[str], query: str, max_length: int = 1000) -> str:
    """
    Merge relevant chunks for query processing.
    ...
    Returns:
        Merged text relevant to the query
    """
    if not chunks:
        return ""
    ...
    for chunk in chunks:
        # Simple relevance: check if query terms appear in chunk
        query_terms = query.lower().split()
        relevance_score = sum(1 for term in query_terms if term in chunk.lower())
        
        if relevance_score > 0 and current_length + len(chunk) <= max_length:
            relevant_chunks.append(chunk)
            current_length += len(chunk)
    
    # If no relevant chunks found, return the first few chunks
